...
import net.royal.spring.framework.web.boot.ConstanteBootDB;
...
@Configuration
public class HibernateConfiguration extends HibernateCore {
	...
	public LocalSessionFactoryBean getSessionFactory() throws IOException {
		...
		bdPorDefecto = "comsatel_generico";

		if (!UString.esNuloVacio(bdPorDefecto))
			configuracion = UPropiedades.getInstance().abrir(recursoRuta,bdPorDefecto + ".properties");
		
		if (configuracion == null)
			configuracion = UPropiedades.getInstance().abrir(recursoRuta,ConstanteBootDB.DATABASE_GENERICO + ".properties");
